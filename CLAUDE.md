...
3. **`extensions/kanban.ts`** — pi extension exposing the `/kanban` slash command (`start | stop | status | open web|app | session open | session pin | session sticky-pin | session unpin | session preview-doc | session link-doc`). Spawns `server.js` as a child process on port 3460 and proxies pin/preview commands via HTTP. This is the entry point when installed via `pi install npm:pi-kanban`.

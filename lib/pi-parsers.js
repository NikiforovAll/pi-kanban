    ...
    if (e.type === 'session') sessionEntry = e;
      ...
      if (m.role === 'user') userCount++;
        ...
        if (m.toolName === 'todo' && m.details && Array.isArray(m.details.tasks)) {
          ...
          if (m.details.results.length === 0 && pendingArr && pendingArr.length) {
...
async function enrichPendingSubagents(meta, subagents) {
  let pending = subagents.filter((r) => !r.runId);
  ...
  pending = subagents.filter((r) => !r.runId);
  if (pending.length === 0) return;
...
async function matchLiveSubagentRuns(meta, pending, allSubagents) {
      ...
      try {
        ...
        for (const line of lines) {
          try {
            ...
            if (ev.type === 'session' && !firstTs) firstTs = ev.timestamp;
            if (!firstUserText && ev.type === 'message' && ev.message && ev.message.role === 'user' && Array.isArray(ev.message.content)) {
              const txt = ev.message.content.find((c) => c && c.type === 'text');
              if (txt && typeof txt.text === 'string') firstUserText = txt.text;
            }
          } catch {}
        }
        liveRuns.push({ runId: d.name, runIndex: parseInt(m[1], 10), jsonl, firstTs, firstUserText, mtime: stat.mtimeMs });
      } catch {}
    }
  }
  ...
  let parentMtime = 0;
  try { parentMtime = (await fs.promises.stat(meta.file)).mtimeMs; } catch {}
  const STALE_MS = 60_000;
  const now = Date.now();
  const parentStale = parentMtime && (now - parentMtime > STALE_MS);
  for (const rec of pending) {
    if (rec.stoppedAt) continue;
    let runMtime = 0;
    if (rec.sessionFile) {
      try { runMtime = (await fs.promises.stat(rec.sessionFile)).mtimeMs; } catch {}
    }
    const runStale = runMtime ? (now - runMtime > STALE_MS) : true;
    if (parentStale && runStale) {
      rec.stoppedAt = new Date(runMtime || parentMtime || now).toISOString();
...
async function listSessions() {
  ...
  for (const f of files) {
    try {
      const summary = await buildSessionSummary(f);

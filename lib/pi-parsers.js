...
// Read first N lines (header) and last K lines (for cost/usage tail) without loading whole file.
    ...
    if (e.type === 'session') sessionEntry = e;
      ...
      if (m.role === 'user') userCount++;
            ...
            if (c && c.type === 'toolCall') {
              ...
              if (c.name === 'subagent') {
                const rec = {
                  ...
                  agent: c.arguments && c.arguments.agent ? c.arguments.agent : null,
                  prompt: c.arguments && c.arguments.task ? c.arguments.task : null,
                ...
                subagents.push(rec);
                if (c.id) subagentByToolCallId.set(c.id, rec);
              }
            }
          }
        }
        ...
        if (m.toolName === 'todo' && m.details && Array.isArray(m.details.tasks)) {
          ...
          const pending = m.toolCallId ? subagentByToolCallId.get(m.toolCallId) : null;
          m.details.results.forEach((r, idx) => {
            const target = idx === 0 && pending
              ? pending
              : (() => {
                  const rec = pending ? { ...pending } : {
...
function tasksFromSnapshot(snapshot, sessionId, project) {
...
async function buildSessionSummary(meta) {
  ...
  return {
    ...
    ...tallyTasks(tasksFromSnapshot(s.lastTodoTasks, id, cwd)),
...
function computeToolDetail(name, params) {
  ...
  let detail = null;
  let full = null;
  if (params.file_path) { detail = basename(params.file_path); full = params.file_path; }
  else if (params.path) { detail = basename(params.path); full = params.path; }
...
async function listTasksForSession(meta) {
  const entries = await readSessionEntries(meta.file);
  const s = summarize(entries);
  const id = (s.sessionEntry && s.sessionEntry.id) || slugFromFile(meta.file);
  const cwd = (s.sessionEntry && s.sessionEntry.cwd) || meta.cwd;
  return tasksFromSnapshot(s.lastTodoTasks, id, cwd);
}
...
async function listAllTasks() {
  ...
  for (const f of files) {
    try {
      const entries = await readSessionEntries(f.file);
      const s = summarize(entries);
      const id = (s.sessionEntry && s.sessionEntry.id) || slugFromFile(f.file);
      const cwd = (s.sessionEntry && s.sessionEntry.cwd) || f.cwd;
      out.push(...tasksFromSnapshot(s.lastTodoTasks, id, cwd));
    } catch {}
  }
  return out;
}
